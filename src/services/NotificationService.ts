...
class NotificationService {
    ...
    constructor() {
        console.log("NotificationService initialized, remember to set the Socket.io instance using setIo(io) before using the service.");
    }
    ...
    async createPostNotification(to : string, from: string, postId: string, type: string) {  //genera notifiche per nuovi commenti o like su un post
        try{
            ...
            if (type !== 'refresh') { // salvo la notifica solo se è un like o comento, se è un refresh emetto solo un evento per aggiornare la pagian real-time
                ...
                this.io.to(to).emit("notification:new", { id: notification._id.toString(), type, postId, fromUser: fromUser.username });  // 2. Emissione evento real-time 
                console.log("Notification emitted to user:", to, "with data:", { id: notification._id.toString(), type, postId, fromUser: fromUser });
            } else {
                this.io.to(to).emit("notification:new", { to, type, postId, fromUser: fromUser.username });  // 2. Emissione evento real-time 
            }
    ...
    async sendPendingNotifications(userId: string, socket: Socket) {
        try{
